...
{
    var processIdString = args.Length > 0 ? args[0] : "11220"; // Replace "1234" with a valid PID for testing

    if(!uint.TryParse(processIdString, out var processId)) { 
        return; 
    }
    ...
    int bytesWritten = 0;
    int lpthreadID = 0;
    IntPtr procHandle = OpenProcess((uint)desiredAccess, false, processId);
